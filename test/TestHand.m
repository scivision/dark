...
function setupAllowedFontNames(tc)
if ispc
  tc.allowedFontName = 'Segoe Print';
else
  tc.allowedFontName = 'xkcd Script';
end
end
end
...
ax = gca;
tc.verifyEqual(ax.FontName, tc.allowedFontName, ...
    sprintf('Unexpected font name: %s', ax.FontName));
tc.verifyEqual(ax.FontSize, 20);
tc.verifyTrue(any(strcmpi(ax.FontAngle, {'normal', 'italic'})), ...
    sprintf('Unexpected font angle: %s', ax.FontAngle));
end
...
for i = 1:size(tc.ExpectedRG, 1)
    ...
    tc.verifyTrue(any(deltas <= tc.Tol), ...
        'Expected line color missing after hand("rg").');
end
end
